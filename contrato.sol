...
contract Loteria {
    ...
    function enviarPalpite(uint palpiteEnviado) payable public { // excetoDono() {
        require(palpiteEnviado >= 1 && palpiteEnviado <= 4, 
        "Você tem que escolher um número entre 1 e 4");

        require(palpites[msg.sender] == 0, 
        "Apenas um palpite pode ser enviado por sorteio");
